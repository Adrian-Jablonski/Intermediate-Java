...
public class Home {
    ...
    private void prepareAttempt(AttemptKind kind) {
        ...
        timeline.setCycleCount(kind.getTotalSeconds());     // Amount of times to run the keyframe
        timeline.getKeyFrames().add(new KeyFrame(Duration.seconds(1), e -> {    // Runs every second
            currentAttempt.tick();
            setTimerText(currentAttempt.getRemainingSeconds());
        }));
    ...
    private void addAttemptStyle(AttemptKind kind) {
        container.getStyleClass().add(kind.toString().toLowerCase());   // Adds a class to a container
    }

    private void clearAttemptStyles() {
        container.getStyleClass().remove("playing");
        for (AttemptKind kind: AttemptKind.values()) {
            container.getStyleClass().remove(kind.toString().toLowerCase());    // Clears all classes
        }
    }
