...
//        usingLambdasInShortForm();
    ...
    // Method used prior to Java 8 Lambdas
    ...
    public static void usingMethodReferences() {
        List<Book> books = Books.all();
        Collections.sort(books, Comparator.comparing(Book::getTitle)); // Use the getTitle method from the class Book
        books.forEach(System.out::println); // Prints each book from books
    }
}
